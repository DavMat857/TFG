...
#Historial
def generar_historial(texto, nombre_archivo):
    try:
...
def obtener_elemento_seleccionado():
    ...
    texto_historial = "\n" +str(archivo) + str(algoritmo) +" __ "+ str(satelite)+" __ "+ str(frecuencia)+ "\n" + str(resultado)  + "\n"
    generar_historial(texto_historial, "historial.txt")
...
def ver_historial():
    try:
        contenido = leer_contenido_archivo("historial.txt")
