...
def graf_figura(filename:dict, sat:str,tiempo = 1) -> None:
    l1 = L1(filename,sat)
    l2 = L2(filename,sat)
    l1menosl2 = f1menosf2(l1,l2)
    
    plt.plot(list(map(lambda x: x*tiempo ,list(l1.keys()))),list(l1.values()),'r.', label = "L1")
    plt.plot(list(map(lambda x: x*tiempo ,list(l2.keys()))),list(l2.values()),'b.', label = "L2")
    plt.plot(list(map(lambda x: x*tiempo ,list(l1menosl2.keys()))),list(l1menosl2.values()),'g.',label = "L1-L2")
    
    plt.title("Comparación L1-L2 con L1 y L2")
    plt.xlabel("Tiempo en segundos")
    plt.ylabel("Unidades de ciclo")
